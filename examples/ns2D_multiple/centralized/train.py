...
# def rollout_and_loss(p, z_init, xi_init, z_target, dyn, t_len):
#     # Using the unroll_controlled method
...
#     # trajectories from unroll_controlled usually have shape (t_steps, ...)
...
def rollout_and_loss(p, z_init, xi_init, z_target, dyn, t_len):
    ...
    # total_loss = track + 0.1 * l_accel + 10.0 * l_bound + 0.0001 * jnp.mean(u_traj**2)
    total_loss = track + 1.0 * l_bound + 0.0001 * effort
    
    return total_loss, (track, effort, final_iou)

@partial(jax.jit, static_argnames=("dyn", "t_len"))
def train_step(p, opt_st, z_init_b, xi_init_b, z_target_b, dyn, t_len):
    batched_loss = jax.vmap(rollout_and_loss, in_axes=(None, 0, 0, 0, None, None))

    def mean_loss_fn(pp):
        losses, auxs = batched_loss(pp, z_init_b, xi_init_b, z_target_b, dyn, t_len)
...
for epoch in trange(start_epoch, epochs, desc="Training"):
    ...
    for batch_idx in range(num_batches):
        ...
        if epoch == epochs - 1 and batch_idx == num_batches - 1:
            ...
            omega_traj, z_traj, xi_traj, u_traj, v_traj = dynamics.unroll_controlled(
                omega_init, z_init_batch[0], z_target_batch[0], xi_init_batch[0], params, t_steps
            )
            metrics_log['sample_xi_traj'] = xi_traj
            metrics_log['sample_v_traj'] = v_traj

        params, opt_state, loss, aux, grad_norm = train_step(
            params, opt_state, z_init_batch, xi_init_batch, z_target_batch, dynamics, t_steps
        )
    ...
    if epoch % 10 == 0:
        ...
        print(
            f"\nEpoch {epoch:03d} | Loss {avg_loss:.5f} | IoU {avg_final_iou:.4f} | "
            f"GradNorm {avg_grad_norm:.3f} | LR {current_lr:.1e} | {epoch_time:.1f}s/ep"
        )
