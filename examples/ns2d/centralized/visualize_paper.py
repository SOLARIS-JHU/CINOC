...
def rollout_uncontrolled(smoke_init, xi_init, rho_target, T_steps, Nx, Ny, dt, buoyancy):
    ...
    def step_fn(carry, _):
        ...
        return (smoke_new, xi), (smoke_new, xi, intensities, push_vel)
    
    _, (smoke_traj, xi_traj, u_traj, v_traj) = jax.lax.scan(
        step_fn, (smoke_init, xi_init), None, length=T_steps
    )
    return smoke_traj, xi_traj, u_traj, v_traj
...
def main():
    ...
    for sample_idx in range(n_samples):
        ...
        smoke_ctrl, xi_ctrl, _, vel_ctrl = unroll_controlled(
        ...
        smoke_unctrl, xi_unctrl, _, _ = rollout_uncontrolled(
