...
def get_batch_actions(a_p, y_batch, xi_batch, key, add_noise=True):
    ...
    # Double vmap across (envs, agents)
...
def fast_imagination_step(s_params, a_p, obs, xi, key):
    ...
    margin = 0.02
    oob_penalty = jnp.maximum(0.0, margin - nxi)**2 + jnp.maximum(0.0, nxi - (1.0 - margin))**2
    rewards = jnp.where(is_invalid, -100.0, -jnp.square(safe_ny[:, :, 10])[..., None] - 10.0 * oob_penalty)
...
print(f"Starting Massively Parallel Training: {'MB-HypeMARL' if USE_MB_HYPEMARL else 'HypeMARL'} (FKPP)")
...
for update_step in range(TOTAL_UPDATES):
    ...
    if python_agent_buffer_size > ENV_BATCH_SIZE:
        ...
        # Build Z locally to conserve buffer memory footprint
        bz = bpe
        bnz = bnpe
...
# Save
with open('models/hypemarl_fkpp_params.msgpack', 'wb') as f:
