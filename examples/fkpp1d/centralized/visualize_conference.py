"""
Conference-Quality Visualization for FKPP1D Centralized DPC
...
def create_comparison_figure(x_grid, z_init, z_target, z_traj_ctrl, z_traj_unctrl, 
    ...
    save_path = f'fkpp_dpc_visualization_ex{example_idx}.png'
...
def create_agent_analysis_figure(xi_traj, u_traj, v_traj, z_traj, z_target, example_idx=1):
    ...
    im = ax4.imshow(np.array(z_traj).T, aspect='auto', cmap='RdBu_r', origin='lower',
                    extent=[0, len(z_traj), 0, 1], vmin=0, vmax=1)
    # Overlay agent paths
    for j in range(n_agents):
    ...
    plt.tight_layout(rect=[0, 0.06, 1, 1])
    save_path = f'fkpp_dpc_agents_ex{example_idx}.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"✓ Saved: {save_path}")
    plt.close()
...
def main():
    print("=" * 60)
    print("  FKPP EQUATION DPC - CONFERENCE VISUALIZATION")
    print("=" * 60)
