"""
Animation Script for FKPP1D Centralized DPC
...
def create_side_by_side_animation(x_grid, z_target, z_traj_ctrl, z_traj_unctrl, 
    ...
    fig.suptitle('Fisher-KPP: DPC Control vs Uncontrolled', fontsize=28, fontweight='bold', y=0.98)
    
    # Initialize plots
    ax1, ax2, ax3, ax4 = axes[0, 0], axes[0, 1], axes[1, 0], axes[1, 1]
    
    # Y-limits for FKPP (bounded [0, 1])
    y_min = 0
    y_max = 1.1
    ...
    ax1.set_ylabel(r'Population $z(x,t)$')
    ...
    ax4.set_xlim([0, T])
    ax4.set_ylim([max(mse_ctrl.min()*0.5, 1e-6), max(mse_ctrl.max(), mse_unctrl.max())*1.2])
    ...
    line_mse_ctrl, = ax4.plot([], [], 'r-', lw=2.5, label='DPC Controlled')
...
def main():
    print("=" * 60)
    print("  FISHER-KPP EQUATION DPC - ANIMATION GENERATOR")
    print("=" * 60)
