...
def parallel_physics_step(z_batch, xi_batch, target_batch, actions, prev_v_batch, key):
    ...
    mean_coll_penalty = jnp.mean(coll_penalty, axis=-1)[:, None]
    
    # 5. Acceleration Penalty
    mean_accel_penalty = jnp.mean(0.1 * jnp.sum(jnp.square(v_batch - prev_v_batch), axis=-1), axis=-1)[:, None]
    
    rewards_batch = -mse - effort - mean_oob_penalty - mean_coll_penalty - mean_accel_penalty
...
def train_chunk(carry, step_indices, z_init_bank, z_target_bank, xi_init_template):
    ...
    def scan_step(carry, step_idx):
        buf, a_p, c_p, ta_p, tc_p, o_a, o_c, z, target, xi, prev_v, steps, rng = carry
        ...
        # 2. Physics Step
        nz, nxi, rew, dones, n_prev_v = parallel_physics_step(z, xi, target, actions, prev_v, phys_k)
        steps += 1
        ...
        prev_v_next = jnp.where(needs_reset[:, None, None], fresh_v, n_prev_v)
        ...
        new_carry = (new_buf, a_p, c_p, ta_p, tc_p, o_a, o_c, z_next, target_next, xi_next, prev_v_next, steps_next, rng)
...
prev_v_init = jnp.zeros((NUM_PARALLEL_ENVS, N_AGENTS, 2))
...
carry = (
    ...
    opt_actor, opt_critic, z_init, target_init, xi_init, prev_v_init, steps_init, key
)

print(f"Starting Massively Parallel Centralized RL Training (Chunked & JITed 2D Heat Equation)...")
