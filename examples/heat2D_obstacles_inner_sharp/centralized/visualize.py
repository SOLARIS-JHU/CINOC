"""
2D Heat Equation Control with Obstacles - Centralized Visualization
...
def main():
    print("=" * 70)
    print("  2D HEAT EQUATION WITH OBSTACLES - CENTRALIZED VISUALIZATION")
    print("=" * 70)
    ...
    try:
        params = load_params(model, 'centralized_params_heat2d_obstacles_inner_sharp.msgpack', n_grid, n_agents)
        print(f"✓ Loaded trained parameters ({n_agents} agents)")
    except FileNotFoundError:
        print("✗ Error: centralized_params_heat2d_obstacles_inner_sharp.msgpack not found")
        return
    ...
    # Add overall title
    fig.suptitle('2D Heat Equation with Obstacles: Centralized DPC Control',
                fontsize=14, fontweight='bold', y=0.98)

    # Save as PDF (vector graphics)
    pdf_path = 'heat2d_obstacles_inner_sharp_centralized_visualization.pdf'
    plt.savefig(pdf_path, format='pdf', dpi=300, bbox_inches='tight')
    print(f"\n✓ Saved: {pdf_path}")

    # Also save as high-res PNG
    png_path = 'heat2d_obstacles_inner_sharp_centralized_visualization.png'
