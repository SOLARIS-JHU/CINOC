"""Training script for Centralized 2D Heat Equation Control with Obstacles using JAX and ControlNet policy."""
import jax
...
# --- Parse Arguments ---
parser = argparse.ArgumentParser(description='Train 2D Heat Centralized Controller with Obstacles')
...
if args.test:
    print("=" * 50)
    print("RUNNING IN TEST MODE (WITH INNER OBSTACLES - SHARP PENALTY)")
    print("=" * 50)
    n_samples = 1
    batch_size = 1
...
R_safe = 0.05  # Collision radius in 2D (reduced from 0.08)

# Obstacle configuration: [x_center, y_center, radius]
# Agents are initialized at: 0.26, 0.42, 0.58, 0.74 in each dimension
# Obstacles placed BETWEEN agent positions
OBSTACLES = jnp.array([
    [0.34, 0.34, 0.05],   # Bottom-left quadrant (between agents)
    [0.50, 0.50, 0.05],   # Center (between four middle agents)
    [0.66, 0.66, 0.05],   # Top-right quadrant (between agents)
])
...
def loss_fn(params, z_init, xi_init, z_target, dynamics):
    ...
    # 4b. Agent-obstacle collision avoidance (SHARP PENALTY - only when inside)
    ...
    # SHARP PENALTY: Only penalize when agent center is INSIDE obstacle (distance < obstacle_radius)
    # No safety margin R_safe - penalty activates only at collision
...
# Save parameters
with open('centralized_params_heat2d_obstacles_inner_sharp.msgpack', 'wb') as f:
    f.write(flax.serialization.to_bytes(params))
print("Parameters saved to centralized_params_heat2d_obstacles_inner_sharp.msgpack")
...
if metrics:
    ...
    plt.tight_layout()
    plt.savefig('training_metrics_heat2d_obstacles_inner_sharp.png')
    print("Training metrics saved to training_metrics_heat2d_obstacles_inner_sharp.png")
