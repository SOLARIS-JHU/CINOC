...
from models_mappo import MAPPOActor2DKS, MAPPOCritic2DKS, U_MAX

# --- Configurations ---
N_AGENTS = 100
L_DOMAIN = 32.0
N_GRID = 64
STATE_NORM_FACTOR = 1.0 
...
PPO_EPOCHS = 4
MINIBATCH_SIZE = 128
TOTAL_UPDATES = 50 #1000
EVAL_INT = 10          # Chunk size for JAX loop
...
def build_marl_obs_batch(u_batch, xi_fixed):
    """Creates a decentralized observation: flattened global state + agent's 2D position."""
    batch_size = u_batch.shape[0]
    
    # Flatten the 2D grid
    u_flat = u_batch.reshape(batch_size, -1) / STATE_NORM_FACTOR
    
    # Tile the global state for each agent (batch, n_agents, N_GRID*N_GRID)
    u_tiled = jnp.tile(u_flat[:, None, :], (1, N_AGENTS, 1))
    
    # Tile the positions for each batch and normalize them (batch, n_agents, 2)
    xi_tiled = jnp.tile(xi_fixed[None, :, :], (batch_size, 1, 1)) / L_DOMAIN
...
def parallel_physics_step(u_batch, actions, xi_fixed):
    ...
    # Pass 'actions' directly instead of 'acts_flat'
    ...
    rewards_batch = jnp.tile(global_energy[:, None], (1, N_AGENTS))
...
def train_step(runner_state, state_bank):
    # MEMORY FIX: Removed obs_batch from the runner_state buffer. 
    actor_state, critic_state, u_batch, env_counts, rng = runner_state
    ...
    def _env_step(carry, _):
        ...
        # MEMORY FIX: Build obs on the fly for execution only, do not save to transitions
        obs = build_marl_obs_batch(u, xi_fixed)
        ...
        # Do not save `obs` to transitions to avoid memory explosion
        transition = (u, action, log_prob, rewards, val, crashes, true_next_v)
    ...
    t_u, t_a, t_logp, t_r, t_v, t_d, t_true_next_v = transitions
    ...
    f_u = t_u.reshape(-1, N_GRID, N_GRID)  # Preserve 2D Grid
    ...
    def _update_epoch(epoch_carry, _):
        ...
        def _update_minibatch(mb_carry, start_idx):
            ...
            # MEMORY FIX: Reconstruct obs chunk solely for the minibatch dynamically
            mb_u = f_u[batch_idx]
            mb_obs = build_marl_obs_batch(mb_u, xi_fixed)
            
            mb_batch = (
                mb_obs, mb_u, f_a[batch_idx], 
    ...
    new_runner_state = (actor_state, critic_state, next_u_batch, next_env_counts, rng)
...
def fast_eval_episode(actor_params, init_u, max_steps):
    def step_fn(state, _):
        ...
        traj = dynamics.unroll_controlled(
            u_init=u_curr, xi_fixed=xi_fixed, u_target=target_state, params=act_flat, 
            t_steps=1, substeps=SUBSTEPS, N_grid=N_GRID, L=L_DOMAIN, dt=DT, sigma=1.2
        )
        next_u = traj[0][-1]
...
# 2D Grid flattens to N_GRID * N_GRID + 2 (for X and Y positions)
dummy_obs_full = jnp.zeros((1, N_AGENTS, (N_GRID * N_GRID) + 2))
...
# Initial state no longer carries the massive tiled observation buffer
initial_runner_state = (
    actor_state, critic_state, u_batch, 
    jnp.zeros(NUM_PARALLEL_ENVS), key
)

print(f"Starting Pure JAX MAPPO 2D Training (Chunked)...")
...
with open('models/mappo_ks2d_params.msgpack', 'wb') as f:
