...
# MATD3 INIT: Actor takes local observation. Critic takes JOINT observation & JOINT action.
dummy_input = jnp.zeros((ENV_BATCH_SIZE, total_input_dim))
...
# --- 3. JIT MATD3 TRAINING COMPONENTS ---
@jax.jit
def update_critic(c_p, ta_p, tc_p, opt_c, x, u, r, nx, d, key):
    # Dimensions: x & nx are (BATCH, N_AGENTS, OBS_DIM). u is (BATCH, N_AGENTS, 2).
    ...
    # Cooperative reward setup: Critic evaluates the average team return
...
def update_actor_and_targets(a_p, c_p, ta_p, tc_p, opt_a, x):
    ...
    def a_loss_fn(p):
        # Apply decentralized actor to get individual actions
        actor_apply_fn = jax.vmap(jax.vmap(actor.apply, in_axes=(None, 0)), in_axes=(None, 0))
        u_pred = actor_apply_fn(p, x)
        
        # Concat into joint action to feed Centralized Critic
        joint_u_pred = u_pred.reshape(batch_size, -1)
        q1, _ = critic.apply(c_p, joint_x, joint_u_pred)
        
        # Maximize Centralized Q-Value (JAX handles the nested backprop automatically!)
        return -jnp.mean(q1)
...
def parallel_marl_physics_step(z_batch, xi_batch, target_batch, actions, key):
    ...
    # --- FULLY LOCAL REWARD UPDATE ---
    # Map continuous positions to discrete spatial grid indices
    agent_indices = jnp.clip((safe_xi * (N_GRID - 1)).astype(jnp.int32), 0, N_GRID - 1)
    batch_indices = jnp.arange(safe_z.shape[0])[:, None]
    
    # Gather only the specific segment of the PDE the agent is currently sitting on
    ...
    # Combine the new local tracking reward with local penalties
    rewards_batch = (r_track_local + r_effort + r_bound + r_coll)[..., None]
...
def train_chunk(carry, step_indices, z_init_bank, z_target_bank):
    def scan_step(carry, step_idx):
        ...
        def do_network_updates(net_state):
            ...
            # Form standard (BATCH, N_AGENTS, OBS_DIM) tensors
...
print(f"Starting Massively Parallel MATD3 Training (Chunked & JITed)...")
start_time = time.time()
