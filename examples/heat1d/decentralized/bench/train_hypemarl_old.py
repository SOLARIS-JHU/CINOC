...
# Provide 1D Dummies for clean module initialization
...
def parallel_marl_physics_step(z_batch, xi_batch, target_batch, actions, key):
    ...
    # --- Recalculating Full Heat Rewards Vectorized ---
    u_batch = actions[..., 0]
    v_batch = actions[..., 1]
    
    # 1. Tracking 
    center_errors = next_obs_batch_no_pe[:, :, 10]
    r_track = -.1 * jnp.square(center_errors)
    ...
    # Combine into unified reward signal
...
def fast_imagination_step(s_params, a_p, obs, xi, key):
    ...
    nxi = nxi_flat.reshape(xi.shape)
    
    # Clip coordinates inside bounds
    nxi = jnp.clip(nxi, 0.0, 1.0)
    
    is_invalid = jnp.logical_not(jnp.isfinite(ny).all(axis=-1, keepdims=True))
    safe_ny = jnp.where(is_invalid, jnp.zeros_like(ny), ny)
    
    # Vectorized Surrogate Reward logic
    u_batch = act[..., 0]
    v_batch = act[..., 1]
    
    r_track = -5.0 * jnp.square(safe_ny[:, :, 10])
    r_effort = -0.001 * (jnp.square(u_batch) + 0.1 * jnp.square(v_batch))
    
    margin = 0.02
    r_bound = -100.0 * (jnp.maximum(0.0, margin - nxi)**2 + jnp.maximum(0.0, nxi - (1.0 - margin))**2)
    
    R_safe = 0.05
    dists = jnp.abs(nxi[:, :, None] - nxi[:, None, :])
    mask = jnp.eye(N_AGENTS)[None, :, :]
    r_coll = -1.0 * jnp.sum(jnp.maximum(0.0, R_safe - (dists + mask * 1.0)) ** 2, axis=2)
    
    r_total = r_track + r_effort + r_bound + r_coll
    rewards = jnp.where(is_invalid, -100.0, r_total)[..., None]
    
    n_obs = safe_ny
    dones = is_invalid 
    
    return n_obs, nxi, act, rewards, dones
...
print(f"Starting Massively Parallel Training: {'MB-HypeMARL' if USE_MB_HYPEMARL else 'HypeMARL'} (Heat Equation)")
...
for update_step in range(TOTAL_UPDATES):
    ...
    agent_buffer = add_batch_to_buffer(agent_buffer, obs_batch, xi_batch, actions, safe_rewards, next_obs_batch, next_xi_batch, dones_expanded)
    rom_buffer = add_batch_to_buffer(rom_buffer, obs_batch, xi_batch, actions, safe_rewards, next_obs_batch, next_xi_batch, dones_expanded)
    ...
    if USE_MB_HYPEMARL and python_rom_buffer_size > ENV_BATCH_SIZE:
        b_s, b_xi, b_act, _, b_ns, b_nxi, _ = sample_buffer(rom_buffer, ENV_BATCH_SIZE, subkey)
        ...
        surrogate_params, opt_surrogate = train_surrogate_step(
            ...
            b_y.reshape(-1, local_y_dim), b_act.reshape(-1, 2), b_xi.reshape(-1, 1),
            b_ny.reshape(-1, local_y_dim), b_nxi.reshape(-1, 1)
        )
        
        if update_step >= WARMUP_UPDATES:
            for _ in range(IMAGINARY_RATIO):
                img_obs, img_xi, _, _, _, _, _ = sample_buffer(rom_buffer, ENV_BATCH_SIZE, subkey)
                
                for _ in range(IMAGINATION_HORIZON):
                    key, subkey = jax.random.split(key)
                    img_nobs, img_nxi, img_act, img_rew, img_done = fast_imagination_step(
                        surrogate_params, actor_params, img_obs, img_xi, subkey
                    )
                    
                    agent_buffer = add_batch_to_buffer(agent_buffer, img_obs, img_xi, img_act, img_rew, img_nobs, img_nxi, img_done)
                    python_agent_buffer_size = min(python_agent_buffer_size + ENV_BATCH_SIZE, 100_000)
                    img_obs, img_xi = img_nobs, img_nxi
    ...
    if python_agent_buffer_size > ENV_BATCH_SIZE:
        ...
        bz_flat = bz.reshape(-1, z_dim)
        bnz_flat = bnz.reshape(-1, z_dim)
        by_flat = by.reshape(-1, local_y_dim)
        bny_flat = bny.reshape(-1, local_y_dim)
        ...
        critic1_params, critic2_params, opt_critic1, opt_critic2 = update_critics(
            ...
            opt_critic1, opt_critic2, by_flat, bz_flat, bu_flat, br_flat, bny_flat, bnz_flat, bd_flat, subkey
        )
