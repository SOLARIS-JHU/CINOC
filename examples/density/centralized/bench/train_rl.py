...
from env_density import NS2DDensityMARLEnv, extract_patches_ns2d_jit
from utils_hypemarl import get_sinusoidal_encoding
from examples.density.centralized.dynamics import unroll_controlled
from models_rl import CentralizedTD3Actor, CentralizedTD3Critic, V_MAX

# =============================================================================
# Load Data & Config (Matches DPC baseline)
# =============================================================================
...
# Physics Constants from DPC Baseline
BUOYANCY = 0.0
SIGMA_PUSH = 0.2
PUSH_MAX = 0.8
R_SAFE = 0.15
...
class DensityDynamicsWrapper:
    """Wraps the functional unroll_controlled to act like the expected dynamics object."""
    def __init__(self, policy_apply_fn):
        self.policy_apply_fn = policy_apply_fn

    def unroll_controlled(self, rho_init, xi_init, rho_target, params, t_steps):
        return unroll_controlled(
            rho_init.astype(jnp.float32), 
            xi_init.astype(jnp.float32), 
            rho_target.astype(jnp.float32), 
            params.astype(jnp.float32), 
            self.policy_apply_fn, t_steps,
            Nx=Nx, Ny=Ny,
            dt=jnp.float32(dt), 
            buoyancy=jnp.float32(BUOYANCY), 
            sigma_push=jnp.float32(SIGMA_PUSH), 
            push_max=jnp.float32(PUSH_MAX), 
        )
...
env = NS2DDensityMARLEnv(dynamics, n_agents=N_AGENTS, Nx=Nx, Ny=Ny, max_steps=MAX_ENV_STEPS)

local_y_dim = env.local_y_dim
n_phys = env.n_phys
pe_dim = PE_D * 2 # 128

stored_obs_dim = local_y_dim + n_phys 
total_input_dim = stored_obs_dim + pe_dim

phys_jax = jnp.array(env.buoyancy)
window_size = env.window_size
resized_dim = env.resized_dim

actor = CentralizedTD3Actor(n_agents=N_AGENTS)
critic = CentralizedTD3Critic(n_agents=N_AGENTS)

key, *subkeys = jax.random.split(key, 4)

dummy_input = jnp.zeros((ENV_BATCH_SIZE, N_AGENTS, total_input_dim))
dummy_act = jnp.zeros((ENV_BATCH_SIZE, N_AGENTS, 2))  # 2D actions

# Flatten inputs for centralized architecture
dummy_joint_input = dummy_input.reshape((ENV_BATCH_SIZE, -1))
dummy_joint_act = dummy_act.reshape((ENV_BATCH_SIZE, -1))

# Initialize with Joint Input
actor_params = actor.init(subkeys[0], dummy_joint_input)
...
def build_marl_obs_batch(rho_batch, target_batch, xi_batch):
    def single_env_obs(state, target, xi):
        y_local = extract_patches_ns2d_jit(state, target, xi, window_size, resized_dim)
        phys_broadcast = jnp.tile(phys_jax, (N_AGENTS, 1))
        return jnp.concatenate([y_local, phys_broadcast], axis=-1)
    return jax.vmap(single_env_obs)(rho_batch, target_batch, xi_batch)
...
def update_critic(c_p, ta_p, tc_p, opt_c, x, u, r, nx, d, key):
    ...
    noise_scale = jnp.array([V_MAX, V_MAX]) * 0.1
    noise = jnp.clip(jax.random.normal(noise_key, u.shape) * noise_scale, -0.5 * noise_scale, 0.5 * noise_scale)
    
    # Flatten inputs into Joint representation
    joint_x = x.reshape(batch_size, -1)
    joint_nx = nx.reshape(batch_size, -1)
    joint_u = u.reshape(batch_size, -1)
    
    # Get Next Action from Joint State
    raw_next_u = actor.apply(ta_p, joint_nx) + noise
    next_u = jnp.clip(raw_next_u, jnp.array([-V_MAX, -V_MAX]), jnp.array([V_MAX, V_MAX]))
    joint_next_u = next_u.reshape(batch_size, -1)

    t_q1, t_q2 = critic.apply(tc_p, joint_nx, joint_next_u)
    
    # Aggregate to Joint Reward and scalar Done
    global_r = jnp.sum(r, axis=1)  # shape (batch, 1)
    global_d = d[:, 0]             # shape (batch, 1)
    
    target_q = global_r + 0.99 * (1.0 - global_d) * jnp.minimum(t_q1, t_q2)
...
def get_batch_actions(a_p, obs_batch_no_pe, xi_batch, key, add_noise=True):
    ...
    # Flatten across agents to feed into the centralized actor
    joint_obs = full_obs.reshape(full_obs.shape[0], -1)
    actions = actor.apply(a_p, joint_obs)
    
    if add_noise:
        noise_scale = jnp.array([V_MAX, V_MAX]) * 0.1
        noise = jax.random.normal(key, actions.shape) * noise_scale
        actions = jnp.clip(actions + noise, jnp.array([-V_MAX, -V_MAX]), jnp.array([V_MAX, V_MAX]))
    return actions

@jax.jit
def parallel_marl_physics_step(rho_batch, xi_batch, target_batch, actions, prev_v_batch, key):
    ...
    def single_physics_step(rho_s, xi_s, target_s, act_s, k_s):
        ...
        return traj[0][-1], traj[1][-1], traj[2][-1]
    
    next_rho_batch, next_xi_batch, v_act = jax.vmap(single_physics_step)(rho_batch, xi_batch, target_batch, actions, keys)
    ...
    next_obs_batch_no_pe = build_marl_obs_batch(safe_rho, target_batch, safe_xi)
    
    # --- COMBINED LOCAL + GLOBAL TRACKING REWARD ---
    x_idx = jnp.clip((safe_xi[..., 0] * (Nx - 1)).astype(jnp.int32), 0, Nx - 1)
    y_idx = jnp.clip((safe_xi[..., 1] * (Ny - 1)).astype(jnp.int32), 0, Ny - 1)
    batch_indices = jnp.arange(safe_rho.shape[0])[:, None]
    
    local_rho = safe_rho[batch_indices, x_idx, y_idx]
    local_target = target_batch[batch_indices, x_idx, y_idx]
    local_mse = jnp.square(local_rho - local_target)
    
    global_mse = jnp.mean(jnp.square(safe_rho - target_batch), axis=(1, 2))[:, None]
    
    # Matching W_TRACK = 10.0
    r_track_combined = -10.0 * local_mse - 10.0 * global_mse 
    
    # 2. Effort Penalty
    r_effort = -0.001 * jnp.sum(jnp.square(v_act), axis=-1)
    
    # 3. Boundary Penalty (W_BOUND = 20.0)
    margin = 0.02
    ...
    # 4. Agent-Agent Collision Penalty (W_COLL = 10.0)
    R_safe = 0.15
    diff = safe_xi[:, :, None, :] - safe_xi[:, None, :, :]
    dists = jnp.sqrt(jnp.sum(diff**2, axis=-1) + 1e-8)
    mask = jnp.eye(N_AGENTS)[None, :, :]
    r_coll = -10.0 * jnp.sum(jnp.maximum(0.0, R_safe - (dists + mask * 10.0)) ** 2, axis=2)

    # 5. Acceleration Penalty (W_ACCEL = 0.05)
    r_accel = -0.05 * jnp.sum(jnp.square(v_act - prev_v_batch), axis=-1)
    
    rewards_batch = r_track_combined + r_effort + r_bound + r_coll + r_accel
    rewards_batch = rewards_batch[..., None]
    
    return safe_rho, safe_xi, next_obs_batch_no_pe, rewards_batch, dones_batch, v_act

# --- 4. THE SCAN-COMPILED TRAINING CHUNK ---
@jax.jit
def train_chunk(carry, step_indices, rho_init_bank, rho_target_bank, xi_init_single):
    def scan_step(carry, step_idx):
        buf, a_p, c_p, ta_p, tc_p, o_a, o_c, rho, target, xi, pv, obs, steps, rng = carry
        ...
        def warmup_actions(_):
            return jax.random.uniform(act_k, (NUM_PARALLEL_ENVS, N_AGENTS, 2), 
                                      minval=jnp.array([-V_MAX, -V_MAX], dtype=jnp.float32), 
                                      maxval=jnp.array([V_MAX, V_MAX], dtype=jnp.float32))
        ...
        # 2. Physics Step
        nrho, nxi, nobs, rew, dones, npv = parallel_marl_physics_step(rho, xi, target, actions, pv, phys_k)
        steps += 1
        ...
        obs_next = build_marl_obs_batch(rho_next, target_next, xi_next)
        ...
        new_carry = (new_buf, a_p, c_p, ta_p, tc_p, o_a, o_c, rho_next, target_next, xi_next, pv_next, obs_next, steps_next, rng)
...
def fast_eval_episode(actor_params, init_rho, init_xi, target_rho, max_steps, key):
    def step_fn(state_tuple, _):
        ...
        obs_no_pe = build_marl_obs_batch(rho_curr[None, ...], target_rho[None, ...], xi_curr[None, ...]) 
...
# 3x3 Agent Template bounds matching centralized baseline config
...
prev_v_batch = jnp.zeros((NUM_PARALLEL_ENVS, N_AGENTS, 2), dtype=jnp.float32)
...
carry = (
    ...
    opt_actor, opt_critic, rho_batch, target_batch, xi_batch, prev_v_batch, obs_batch, env_step_counts, key
)
...
with open('models/marl_ns2d_params.msgpack', 'wb') as f:
