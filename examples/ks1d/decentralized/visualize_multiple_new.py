...
def plot_energy_row(ax, t, u_nat, u_ctrl, show_legend=False):
    ...
    ax.plot(t, e_nat, color='grey', linestyle='--', lw=1.5, label='Natural')
    ax.plot(t, e_ctrl, color='navy', lw=1.5, label='Controlled')
...
if __name__ == "__main__":
    ...
    # Increase top margin to make room for colorbar
    fig = plt.figure(figsize=(16, 3.5 * num_scenarios))
    
    # 3 columns: Heatmap, Heatmap, Energy
    # wspace=0.35 protects the overlap
    gs_outer = gridspec.GridSpec(num_scenarios, 3, width_ratios=[1, 1, 0.8], 
                                 wspace=0.35, hspace=0.3,
                                 top=0.90) # Leave space at top for manual colorbar
    
    # Variable to store one image for the colorbar reference
    ref_im = None
    ...
    for i, scen in enumerate(SCENARIOS):
        ...
        # Label left axis with scale info
        ...
        ax_en.set_ylabel(r"Energy $\langle u^2 \rangle$", fontsize=11)
        
        if i == 0:
            ax_en.set_title("Stabilization Performance", fontsize=12, fontweight='bold', pad=10)

    # --- MANUAL COLORBAR PLACEMENT ---
    # This ensures it doesn't shrink the top axes, keeping all rows equal size.
    # Position: [left, bottom, width, height] in figure coordinates
    cbar_ax = fig.add_axes([0.15, 0.94, 0.4, 0.015]) 
    
    cb = fig.colorbar(ref_im, cax=cbar_ax, orientation='horizontal')
    cb.set_label(r'State field $u(x,t)$', rotation=0, labelpad=-1, fontsize=10)
    cbar_ax.xaxis.set_label_position('top')
    cbar_ax.xaxis.set_ticks_position('top')
    
    save_path = "ks1d_multiscale_comparison.pdf"
