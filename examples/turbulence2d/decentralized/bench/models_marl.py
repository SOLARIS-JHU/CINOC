...
# Action scaling constraint (Matches DecentralizedTurbulenceNet default config)
U_MAX = 75.0  

class MARLActor2DTurb(nn.Module):
    """
    Standard Decentralized Actor adapted for 2D Turbulence (Fixed Actuators).
    ...
    def __call__(self, x):
        ...
        # Prevents weird behavior as vorticity approaches zero
        x = x / (jnp.linalg.norm(x, axis=-1, keepdims=True) + 1.0)
        
        # Single Head for Forcing (u), no velocity since agents are fixed
        init_fn = nn.initializers.normal(stddev=0.5)
        u_raw = nn.Dense(1, kernel_init=init_fn)(x)
...
class MARLCritic2DTurb(nn.Module):
    """
