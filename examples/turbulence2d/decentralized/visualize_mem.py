...
CONFIG = {
    # --- 1. Grid Resolution ---
    'N_grid': 64,           # <--- CHANGED: Down from 128
    'L_domain': 1.0,
    
    # --- 2. Physics (Coarser Grid = More Stable) ---
    'dt': 0.01,             # Keep high control frequency
    'substeps': 5,          # <--- CHANGED: 5 is sufficient for N=64 (dt_phys=0.002s)
    'viscosity': 5e-4,      # <--- CHANGED: Higher viscosity for lower resolution
    
    'n_agents': 64,
    'grid_shape': (8, 8),
    'sigma': 0.05,          # <--- CHANGED: Slightly wider sigma prevents pixel artifacts
    
    # --- 3. Duration (Same physical time) ---
    'T_chaos_steps': 50,    # 0.5s
    'T_control_steps': 200, # 2.0s
    
    'snapshot_times': [-0.25, 0.0, 0.5, 1.0, 2.0],
    
    # --- 4. Files ---
    'params_file': 'turbulence_params.msgpack',       # Ensure this matches your save name
    'ic_filename': 'turbulence_chaotic_ics_64_real.pkl', # Ensure you generated 64x64 data
}
...
if __name__ == "__main__":
    print("--- Visualizing Exact Training Example (64x64) ---")
    
    model = DecentralizedTurbulenceNet(
        features=(32, 64), 
        domain_size=(CONFIG['L_domain'], CONFIG['L_domain']),
        # --- MATCH TRAINING HYPERPARAMS ---
        u_max=150.0,      # <--- CHANGED: Match the aggressive training force
        patch_size=16     # <--- CHANGED: 16px on 64-grid = same physical area as 32px on 128-grid
    )
    
    try:
        # Load params and 64x64 Data
        params = load_params(model, CONFIG['params_file'])
        w0_hat = load_exact_training_ic()
    except Exception as e:
        print(f"Error: {e}")
        print("Did you run the 64x64 data generation and training first?")
        sys.exit(1)

    t_axis, w_ctrl, w_base, _ = run_comparison(w0_hat, model, params)
    
    plot_exact_result(t_axis, w_ctrl, w_base, save_name="exact_result_64x64.png")