...
def load_params(model, filepath, n_pde=100, n_agents=4):
    ...
    # The 3-arg signature matches DecentralizedControlNet __call__
...
def visualize_rollout(params, model, z_init, xi_init, z_target, dynamics, T_steps=300):
    def step_fn(carry, _):
        ...
        # Decentralized inference happens here
...
def main():
    n_pde, n_agents, T_steps = 100, 5, 300 # 5 agents for decentralized
    ...
    with solver_ts:
        ...
        try:
            ...
            print("Loaded decentralized model parameters.")
        ...
        for i in range(2): 
            ...
            # FEATURE: Faint intermediate steps (Ghosting)
            for t in range(0, T_steps, 10):
                ax.plot(x_grid, z_traj[t], 'g-', alpha=0.1)
                
            ax.plot(x_grid, z_traj[-1], 'r-', label='Final Output', linewidth=2)
            
            # Highlight final actuator positions
            actuator_indices = jnp.clip((xi_traj[-1] * n_pde).astype(int), 0, n_pde-1)
            ax.scatter(xi_traj[-1], z_traj[-1, actuator_indices], color='red', zorder=5, label='Actuators')
            
            ax.set_title(f"Ex {i+1}: State Evolution (Decentralized)")
            ax.set_ylim([-2, 2]) # FEATURE: Fixed Y-limit
            ax.legend()
        ...
        plt.tight_layout()
        plt.savefig('decentralized_visualization.png')
        print("Saved updated visualization to decentralized_visualization.png")
