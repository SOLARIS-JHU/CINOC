...
def forcing_fn(xi, u):
    """
    Gaussian actuator forcing field with Periodic (Torus) distance.
    """
    # 1. Compute raw displacement
    # Broadcast: (M, 1, 1) - (1, N, N) -> (M, N, N)
    dx = xx[None, :, :] - xi[:, 0, None, None]
    dy = yy[None, :, :] - xi[:, 1, None, None]
    
    # 2. Minimum Image Convention
    # On a torus of length L, the max distance is L/2. 
    # If dx > L/2, it's closer to wrap around the other way.
    dx = (dx + L/2) % L - L/2
    dy = (dy + L/2) % L - L/2
    
    # 3. Compute Gaussian basis using the corrected shortest distance
    sq_dist = dx**2 + dy**2
    basis = jnp.exp(-sq_dist / (2.0 * sigma**2))
    
    # 4. Sum contributions
    fx = jnp.sum(u[:, 0, None, None] * basis, axis=0)
    fy = jnp.sum(u[:, 1, None, None] * basis, axis=0)
...
def full_step(omega_hat, rho, xi, u, v):
    ...
    # # This ensures the density field doesn't develop spectral ringing
    # rho_hat_new = jfft.rfft2(rho_new)
    # rho_new = jfft.irfft2(rho_hat_new * FILTER)
    ...
    grad_omega_x, grad_omega_y = spectral_gradient(omega_hat)
    advection = -(vx * grad_omega_x + vy * grad_omega_y)
    advection_hat = DEALIAS * jfft.rfft2(advection)
    
    # Compute forcing curl: (∇×f)_z
    fx, fy = forcing_fn(xi, u)
    curl_f_hat = DEALIAS * spectral_curl_z(fx, fy)
    
    # Compute buoyancy force contribution: rot([0, B*rho]) = B * d(rho)/dx
    if use_buoyancy:
        rho_hat = jfft.rfft2(rho)
        two_pi_i = 2.0 * jnp.pi * 1j
        buoyancy_hat = DEALIAS * (BUOYANCY_STRENGTH * (two_pi_i * KX * rho_hat))
    else:
        buoyancy_hat = 0.0
...
def solve_with_policy(omega_hat_init, rho_init, rho_target, xi_init, params, policy_apply_fn, t_steps):
    def step_fn(carry, _):
        omega_hat, rho, xi = carry
        
        # Policy sees physical field
        u, v = policy_apply_fn(params, rho, rho_target, xi)
        
        # Physics advances state
        omega_hat_next, rho_next, xi_next = full_step(omega_hat, rho, xi, u, v)
        
        # Carry is (3-tuple), Trajectory is (5-tuple)
        return (omega_hat_next, rho_next, xi_next), (omega_hat_next, rho_next, xi_next, u, v)

    _, trajectory = jax.lax.scan(
        step_fn, (omega_hat_init, rho_init, xi_init), None, length=t_steps
    )
    return trajectory
