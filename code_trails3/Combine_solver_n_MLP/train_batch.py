...
def rollout_fn(params, z_init_batch, xi_init_batch, z_target_batch, dynamics):
    """
    Simulates batched trajectories.
    Args:
    ...
    Returns:
        z_traj: (B, T, N)
        xi_traj: (B, T, n_agents)
        u_traj: (B, T, n_agents)
        v_traj: (B, T, n_agents)
    """
    # Vectorize over batch dimension
    rollout_batched = jax.vmap(
        lambda z_i, xi_i, z_t: rollout_single(params, z_i, xi_i, z_t, dynamics),
        in_axes=(0, 0, 0)
    )
    
    return rollout_batched(z_init_batch, xi_init_batch, z_target_batch)


def loss_fn(params, z_init_batch, xi_init_batch, z_target_batch, dynamics):
    """
    Computes loss over a batch of trajectories.
    """
    z_traj, xi_traj, u_traj, v_traj = rollout_fn(params, z_init_batch, xi_init_batch, z_target_batch, dynamics)
    
    # z_traj: (B, T, N), z_target_batch: (B, N)
    # Broadcast target across time: (B, 1, N)
    z_target_expanded = z_target_batch[:, None, :]
    
    # 1. Tracking Loss: MSE between z_traj and z_target
    l_track = jnp.mean((z_traj - z_target_expanded) ** 2)
    ...
    # xi_traj: (B, T, n_agents)
    # Compute pairwise distances for each batch and time step
    # Expand dims to (B, T, N, 1) and (B, T, 1, N)
    xi_exp = xi_traj[:, :, :, None]  # (B, T, n_agents, 1)
    xi_t_exp = xi_traj[:, :, None, :]  # (B, T, 1, n_agents)
    dists = jnp.abs(xi_exp - xi_t_exp)  # (B, T, n_agents, n_agents)
    
    # Avoid self-collision (diagonal is 0)
    mask = jnp.eye(n_agents)[None, None, :, :]  # (1, 1, n_agents, n_agents)
    dists = dists + mask * 1.0
    
    # Penalty: max(0, R_safe - dist)^2
    coll_penalty = jnp.sum(jnp.maximum(0, R_safe - dists) ** 2, axis=(2, 3))  # (B, T)
